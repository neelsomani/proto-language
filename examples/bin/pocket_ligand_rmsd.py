...
def compute_ligand_rmsd_pymol(ref_pdb, design_pdb, ligand_name, pocket_distance):
    ...
    ref_atoms = {}
    cmd.iterate_state(1, f"ref and resn {ligand_name}",
                      "ref_atoms[name] = (x, y, z)",
                      space={'ref_atoms': ref_atoms})

    design_atoms = {}
    cmd.iterate_state(1, f"design and resn {ligand_name}",
                      "design_atoms[name] = (x, y, z)",
                      space={'design_atoms': design_atoms})
...
Examples:
    ...
    parser.add_argument(
        "reference",
        help="Reference PDB file with ligand"
    )
    parser.add_argument(
        "designs",
        nargs="*",
        help="Design PDB files (supports shell wildcards)"
    )
    parser.add_argument(
        "-g", "--glob",
        action="append",
    ...
    parser.add_argument(
        "-l", "--ligand",
        default="ATP",
        help="Ligand residue name (default: ATP)"
    )
    parser.add_argument(
        "-d", "--pocket-distance",
        type=float,
    ...
    parser.add_argument(
        "-o", "--output",
        default="pocket_ligand_rmsd_output.csv",
        help="Output CSV file for results"
    )
    parser.add_argument(
        "-q", "--quiet",
        action="store_true",
        help="Suppress per-structure output"
    )
    ...
    for design_path in design_pdbs:
        ...
        try:
            result = compute_ligand_rmsd_pymol(
                args.reference,
                design_path,
                args.ligand,
                args.pocket_distance
            )
    ...
    if args.output:
        ...
        with open(args.output, 'w', newline='') as f:
            fieldnames = [
                'design', 'ligand_rmsd', 'alignment_rmsd', 'alignment_length',
                'n_ligand_atoms', 'ref_pocket_size', 'design_pocket_size', 'status'
            ]
