###########################################
### GENOME DESIGN // FILTERING PIPELINE ###
###########################################
...
conda activate sk_evo_exp
CONFIG_FILE="/path/to/config.yaml"
python /path/to/genome_design_filtering_pipeline.py $CONFIG_FILE

Requirements:
- Evo environment equivalent to sk_evo_exp
- lovis4u environment equivalent to lovis4u_exp
...
def valid_mmseqs_pident_deprecated(mmseqs_df: pd.DataFrame, descriptive_prefix: str, pident_range: tuple, sequences_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop the '_ORF.#' from orfipy prediction results if applicable to mmseqs_df, and de-duplicate sequence IDs keeping only max sequence identity rows
    ...
    # Filter the DataFrame based on the specified % identity range
    results_df = mmseqs_df[(mmseqs_df[f'{descriptive_prefix}_mmseqs_percent_identity'] >= min_pident) & (mmseqs_df[f'{descriptive_prefix}_mmseqs_percent_identity'] <= max_pident)]
    ...
    for index, row in results_df.iterrows():
        ...
        # Replace the id_prompt in results_df with the matching one from sequences_df
        if not matching_prompt.empty:
            results_df.at[index, 'id_prompt'] = matching_prompt.values[0]
    
    # Add results and remove rows not in range
...
def valid_protein_database_hit_count_deprecated(protein_database_hits_df: pd.DataFrame, sequences_df: pd.DataFrame, id_column: str='query', min_hits: int=7) -> pd.DataFrame:
    """
    Filters the sequences dataframe to keep only entries with genome IDs that have at least a specified number of MMseqs hits to a given protein database.
    ...
    # Initialize a column for hit counts in sequences_df
    sequences_df['protein_database_hit_count'] = 0
    
    # Match and assign hit counts using substrings
    for genome_id in genomes_to_keep:
        mask = sequences_df['id_prompt'].str.contains(genome_id, regex=False)
        sequences_df.loc[mask, 'protein_database_hit_count'] = genome_counts[genome_id]
    
    if min_hits == 0:
        return sequences_df
    else:
        # Filter sequences DataFrame based on the minimum hit count
        filtered_df = sequences_df[sequences_df['protein_database_hit_count'] >= min_hits].copy()
        return filtered_df
...
def valid_gene_annotations_deprecated(input_gff_dir: str, input_gbk_dir: str, required_products: tuple) -> None:
    """
    Validates gene annotations and deletes directories that do not contain the required gene products.
    ...
    for gff_file in gff_files:
        ...
        # Check if all required products are present (allowing extra ones)
        if all(product in extracted_products for product in required_products):
            ...
            # Delete corresponding GBK file
            gbk_file = os.path.join(input_gbk_dir, f"{genome_id}.gbk")
            if os.path.exists(gbk_file):
                os.remove(gbk_file)
                print(f"Deleted: {gbk_file}")

            # Delete the entire directory if it exists (assuming each genome has its own folder)
