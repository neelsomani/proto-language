...
from proto_language.language.generator.generator_registry import GeneratorRegistry, GeneratorSpec
...
class TestGeneratorBase:
    ...
    def test_assign_rejects_constant_segment(self):
        """Tests that assign raises error for constant segments."""
        gen = ConcreteGenerator()
        segment = Segment(sequence="ATCG", sequence_type="dna", constant=True)
    ...
    def test_assign_mutation_generator_initializes_random_sequence(self):
        """Tests that mutation generators initialize a random sequence if none provided."""
        gen = ConcreteGenerator()
        seq_len = 20
        ...
        mock_spec.category = "mutation"  # Mutation generators initialize random sequences
        
        with patch.object(GeneratorRegistry, 'get', return_value=mock_spec):
            with patch.object(GeneratorRegistry, 'get_key', return_value='concrete-generator'):
                gen.assign(segment)
        
        # Should now have a random sequence of the correct length
        assert len(segment.original_sequence.sequence) == seq_len
        # All characters should be valid DNA nucleotides
        assert all(c in "ACGT" for c in segment.original_sequence.sequence)

    def test_assign_mutation_generator_preserves_existing_sequence(self):
        ...
        # Should preserve the original sequence
        assert segment.original_sequence.sequence == predefined_seq
