...
_NA_MAP = {"DA": "A", "DT": "T", "DG": "G", "DC": "C", "DI": "I", "A": "A", "U": "U", "G": "G", "C": "C", "I": "I"}
...
def find_pdb_file(pdb_dir: Path, pdb_id: str) -> Path | None:
    ...
    proposals = [
        f"{clean_id}.pdb",
        f"{clean_id}.cif",
        f"{clean_id.lower()}.pdb",
        f"pdb{clean_id.lower()}.ent",
    ...
    for run in json_objects:
        ...
        for complex_idx, complex_info in enumerate(complexes):
            ...
            for ref_pdb in list(pdb_comps.keys()):
                ...
                af3_pdb_path = f"{run_dir}/af3_outputs_af3_results{af3_pdb_suffix}/af3_job_0_af3.pdb"
                ...
                rows.append(
                    {
                        ...
                        "ref_dna_seqs": (str(scan_res["dna_seqs"]) if scan_res["has_dna"] else ""),
                        "ref_rna_seqs": (str(scan_res["rna_seqs"]) if scan_res["has_rna"] else ""),
...
def analyze_and_report(df, used_pdb_dir=False):
    ...
    if not oom_runs.empty:
        ...
        print(
            oom_runs[["complex_id", "n_subunits", "total_residues"]]
    ...
    if not chain_limit_runs.empty:
        ...
        print(
            chain_limit_runs[["complex_id", "n_subunits", "total_residues"]]
    ...
    for idx, row in df.iterrows():
        if row["complex_id"] in confirmed_ids:
            continue
